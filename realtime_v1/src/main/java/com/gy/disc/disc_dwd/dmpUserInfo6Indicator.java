...
//        userInfoSupMsgTableData.print();
        ...
        SingleOutputStreamOperator<JSONObject> userInfoJoinUserSumMsgDs = UserInfo
                ...
                .uid("intervalJoin")
                .name("intervalJoin");
//        {"birthday":"1970-07-07","uname":"魏淑","gender":"home","weight":"63","uid":"740","login_name":"z5e0xq","constellation":"巨蟹座","unit_height":"cm","ear":1970,"user_level":"1","phone_num":"13511947252","unit_weight":"kg","email":"z5e0xq@live.com","ts_ms":1747016022083,"height":"178"}
...
//        order_detail_sku.print();
        ...
        SingleOutputStreamOperator<JSONObject> categoryJoinTmDs = order_detail_sku_category
                ...
                .process(new ProcessJoinFunction<JSONObject, JSONObject, JSONObject>() {
                    @Override
                    public void processElement(JSONObject j1, JSONObject j2, ProcessJoinFunction<JSONObject, JSONObject, JSONObject>.Context context, Collector<JSONObject> collector) throws Exception {
                        j1.put("tm_name", j2.getString("tm_name"));
                        collector.collect(j1);
...
//        categoryJoinTmDs.print();
        ...
        * {"category1_id":"8","tm_name":"索芙特","uid":"57","category2_name":"香水彩妆","tm_id":"8","category1_name":"个护化妆","category3_name":"唇部","category3_id":"477","category2_id":"54"}
        ...
        SingleOutputStreamOperator<JSONObject> UserINfoJoinCategoryDs = win2MinutesPageLogsDs
                .keyBy(data -> data.getLong("uid"))
                .intervalJoin(categoryJoinTmDs.keyBy(data -> data.getLong("uid")))
                .between(Time.minutes(-60), Time.minutes(60))
                .process(new ProcessJoinFunction<JSONObject, JSONObject, JSONObject>() {
                    @Override
                    public void processElement(JSONObject j1, JSONObject j2, ProcessJoinFunction<JSONObject, JSONObject, JSONObject>.Context context, Collector<JSONObject> collector) throws Exception {
                        j1.put("category1_name", j2.getString("category1_name"));
                        j1.put("tm_name", j2.getString("tm_name"));
                        collector.collect(j1);
                    }
                })
...
//        userInfoYearScoringModelDS.print("打分模型");
        ...
        SingleOutputStreamOperator<JSONObject> name = userInfoJoinUserSumMsgDs
                .keyBy(data -> data.getLong("uid"))
                .intervalJoin(orderInfoJoinOrderDetailDs.keyBy(data -> data.getLong("user_id")))
                .between(Time.minutes(-60), Time.minutes(60))
                .process(new ProcessJoinFunction<JSONObject, JSONObject, JSONObject>() {
                    @Override
                    public void processElement(JSONObject j1, JSONObject j2, ProcessJoinFunction<JSONObject, JSONObject, JSONObject>.Context context, Collector<JSONObject> collector) throws Exception {
                        j1.put("order_id", j2.getString("id"));
                        j1.put("total_amount", j2.getLong("total_amount"));
                        j1.put("create_time", j2.getString("create_time"));
                        j1.put("sku_id", j2.getLong("sku_id"));
                        collector.collect(j1);

                    }
                })
                .uid("user join page date")
                .name("user join page date");

//        name.print("userInfo join orderInfo");
